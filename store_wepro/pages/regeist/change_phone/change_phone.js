...
var isPhone=nation.check(1,''+phonesa+'');
  ...
  formSubmit: function (e) {
    ...
    var arr=[];
        ...
        // 到这里数据校验已完成
    app.req('post', "/store/Ucenter/editPhoneStep1", {
      data: { 'token': app.getToken(), 'phone': phonenumber,'verify_code':mescode},
      success: function (data) {
        if (data.code == 200) {
          var hash=data.data.hash;
           nation.urlto('../../regeist/new_phone/new_phone?hash='+hash+'&oldphone='+phonenumber);
        } else {
          util.toast(data.msg);
        }
      }

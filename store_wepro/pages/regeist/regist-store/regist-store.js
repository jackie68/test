...
Page({
  ...
  onLoad: function (event) {
    ...
    //获取经销商
    var that = this;
    app.req('post', "/store/Index/allDealer", {
      data: { 'token': app.getToken() },
      success: function (res) {
        if (res.code == 200) {
          that.setData({
            'distribuArray': res.data,
            'distritId': res.data.dealer_id[0]
          })
  ...
  bindPickerChange: function (event) {
    var dealer_id = this.data.distribuArray.dealer_id[event.detail.value];
    this.setData({
  ...
  formSubmit: function (event) {
    ...
    if (phoneNumber == '') {
      ...
      app.req('post', "/store/Index/reg", {
        data: { 'token': app.getToken(), 'name': phoneNumber, 'pass': passwordValue, 'verify_code': codeValue, 'address': addressValue, 'true_name': nameValue, 'id_card': cardIdNumber, 'store_name': shopName, 'dealer_id': distributorId, 'openid': openid, 'reg_type': '1' },
        success: function (data) {
          if (data.code == 200) {
            util.toast('注册成功');
            setTimeout(function () {
